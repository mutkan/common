...
public class BasePresenter<V extends IBaseView> extends MvpPresenter<V> {
    ...
    protected void onFirstViewAttach() {
        ...
        for (int id : mProgresses.keySet()) {
            safeRun( () -> getViewState().setProgress(ProgressAction.HIDE, IBaseView.ProgressType.withId(id)));
        }
        mProgresses.clear();
    }
    ...
    protected <D> SingleTransformer<D, D> withSingleProgress(IBaseView.ProgressType progress) {
    ...
    protected <D> SingleTransformer<D, D> withSingleProgress() {
        return withSingleProgress(IBaseView.ProgressType.DEFAULT);
    }
    ...
    protected <D> SingleTransformer<D, D> withDefaultSingle() {
        return withDefaultSingle(IBaseView.ProgressType.DEFAULT);
    }

    protected <D> SingleTransformer<D, D> withDefaultSingle(IBaseView.ProgressType progressType) {
        return upstream -> upstream
    ...
    protected <D> FlowableTransformer<D, D> withFlowableProgress(IBaseView.ProgressType progress) {
        return upstream -> upstream
    ...
    protected <D> FlowableTransformer<D, D> withFlowableProgress() {
        return withFlowableProgress(IBaseView.ProgressType.DEFAULT);
    }
    ...
    protected <D> FlowableTransformer<D, D> withDefaultFlowable(IBaseView.ProgressType progressType) {
        return upstream -> upstream
    ...
    protected <D> FlowableTransformer<D, D> withDefaultFlowable() {
        return withDefaultFlowable(IBaseView.ProgressType.DEFAULT);
    }
    ...
    protected void publishProgress(IBaseView.ProgressType progressType) {
    ...
    private void onProgressStart(IBaseView.ProgressType progressType) {
        safeRun( () -> {
            Integer count = mProgresses.get(progressType.getId());
            if (null == count) {
                count = 0;
    ...
    private void onProgressFinish(IBaseView.ProgressType progressType) {
        safeRun( () -> {
            Integer count = mProgresses.get(progressType.getId());
            if (null == count || count == 0) {
                return;
